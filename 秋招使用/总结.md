...
### 问题   guest下polling的话如果来中断怎么处理呢？
可以设置直通，这样就和pcpu无关了，但是如果没那么多设备资源的话，不能进行直通怎么办呢？pcpu等polling完在设置中断这些的话就更浪费性能了。。。。
## ARM TLBI优化
vcpu和pcpu相差太多的情况下，再使用TLBI IS的方式性能就八太行了,TLBI IS就是类似TLB shootdown的形式，给其他CPU发IPI，再具体的没有找到，因为那就是指令级别了    
然后就是如果使用TLB shootdown的形式的话，可能要产生多个vm exit来发送IPI，后续可以参照send IPI的优化方式减少为一个vm exit      
TLBI flush的场景（文件在arch\arm64\include\asm\tlbflush.c文件中）：
1. local_flush_tlb_all：不用管，没有使用IS，比如进程上下文切换的时候可能会用吧
2. flush_tlb_all：查了一下没看到使用场景
3. flush_tlb_mm：无应用场景
4. copy_process -> copy_mm -> dup_mmap -> copy_page_range -> copy_hugetlb_page_range -> huge_ptep_set_wrprotect -> get_clear_flush -> flush_tlb_range（copy_process中要copy_mm,如果那个flag是CLONE_VM，说明父子运行在同一地址空间，即创建的其实是一个线程，如果不是的话，说民是一个新的子进程，要执行dup_mm将父进程的mm复制一份给子进程，在dup_mm中会分配一个mm，然后尝试为其填充父进程的内容，这里会调用一个dum_mmap函数，然后有个VM_WIPEONFORK标志，表示不会从父进程VMA中复制页表到子进程VMA，然后如果需要复制的话，就会调用copy_page_range函数在往后往后会调用flush_tlb_range，这个调用的tlbi指令中有IS参数）
5. __set_fixmap -> flush_tlb_kernel_range,,然后有行注释说irq中断可能也会调用__setfixmap，所以需要IPI发送TLB？？
6. init\main.c -> mark_rodata_ro -> flush_tlb_kernel_range,,
7. sram_exec_copy(copy data to a protected executable region of sram) -> set_memory_x/set_memory_nx/set_memory_ro/set_memory_rw ->change_memory_common -> __change_memory_common -> flush_tlb_kernel_range
8. 获取cpu featrue的时候有一个cpu_enable，进入一段汇编，引导期间被每个CPU从停止的机器上执行一次
...
6. 创建0号进程（唯一一个没有通过 fork 或者 kernel_thread产生的进程，采用了gcc的结构体初始化方式为其进行了直接赋值生成），0号进程用于包括内存、页表、必要数据结构、信号、调度器、硬件设备等的初始化。然后生成1号进程和2号进程(以Linux系统为例，先载入/boot目录下面的kernel。内核加载成功后，第一个运行的程序是/sbin/init。它根据配置文件（Debian系统是/etc/initab， CentOS 是systemd）产生init进程。这是Linux启动后的第一个进程，pid进程编号为1，其他进程都是它的后代。

然后，init线程加载系统的各个模块，比如窗口程序和网络程序，直至执行/bin/login程序，跳出登录界面，等待用户输入用户名和密码。)，最后退化为idle进程。，，，中断初始化、内存初始化、调度初始化、res_init(区分内核态用户态，初始化1、2号进程)
...
# 虚拟化一些内容
## CPU steal time
是指CPU需要执行命令但是没有抢到物理CPU来执行的时间（用到了RCU锁来保护stole_time ）    
pv_time_init中检查是否有相关特性，有的话则使能。使用pv_ops.time.steal_clock = pv_steal_clock;设置回调函数，设置回调函数，kernel中就会调用paravirt_steal_clock 来计算steal time   
具体过程（依据patch）：   
1. 
## cpuidle haltpoll driver and governor （https://patchwork.kernel.org/project/kvm/cover/20190703235124.783034907@amt.cnet/）
具体过程：
1. 设置一个HALT_POLL config，然后设置函数init，并注册一个CPUIDLE driver驱动，在CPUIDLE driver struct中有个state数组，表示可以不同的进入idle的方式？
2. add poll_limit_ns to cpuidle_device structure
3. 添加halt poll governor，结构体赋值创建一个governor，init初始化并注册这个governor，然后添加一些类似kvm侧的halt polling的halt_poll_ns这些功能
